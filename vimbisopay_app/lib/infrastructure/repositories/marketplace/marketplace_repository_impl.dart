...
class MarketplaceRepositoryImpl implements MarketplaceRepository {
        ...
        (failure) async {
          if (!isRetry &&
            ...
            return loginResult.fold(
              (loginFailure) {
                Logger.error(
                    '[MARKETPLACE] Token refresh failed: ${loginFailure.message}');
                return Left(loginFailure);
              },
              (newUser) async {
                Logger.data(
                    '[MARKETPLACE] Token refreshed successfully, retrying request');
  ...
  Future<Either<Failure, bool>> enableVendorFunctionality({
    ...
    return _executeAuthenticatedRequest<bool>(
      request: (token) async {
        ...
        Logger.data(
            '[MARKETPLACE] Sending POST request to $_baseUrl/sellInMarket');
        ...
        if (response.statusCode == 200 || response.statusCode == 201) {
          ...
          try {
            isVendor =
                responseData['data']['dashboard']['member']['vendor'] ?? false;
            Logger.data('[MARKETPLACE] Extracted vendor status: $isVendor');
          } catch (e) {
            Logger.error(
                '[MARKETPLACE] Error extracting vendor status from response',
                e);
          ...
          try {
            ...
            if (user != null) {
              ...
              Logger.data(
                  '[MARKETPLACE] Updated user with activateMarket: $isVendor');
            }
          } catch (e) {
            Logger.error(
                '[MARKETPLACE] Error updating user with vendor status', e);
            // Continue even if update fails, as the API call was successful
          }

          stopwatch.stop();
          Logger.performance(
              '[MARKETPLACE] enableVendorFunctionality completed successfully in ${stopwatch.elapsedMilliseconds}ms');
  ...
  Future<Either<Failure, bool>> updateMemberWithVendorDetails({
    ...
    final stopwatch = Stopwatch()..start();
    Logger.data(
        '[MARKETPLACE] Starting updateMemberWithVendorDetails operation');

    // Log the input parameters
    Logger.data(
        '[MARKETPLACE] Update parameters: firstname=$firstname, lastname=$lastname, memberHandle=$memberHandle, vendorBio=${vendorBio != null ? '${vendorBio.substring(0, min(20, vendorBio.length))}...' : 'null'}');
    ...
    return _executeAuthenticatedRequest<bool>(
      request: (token) async {
        ...
        Logger.data(
            '[MARKETPLACE] Sending POST request to $_baseUrl/editMember');
        final response = await _loggedRequest(
          () => _httpClient.post(
        ...
        if (response.statusCode == 200 || response.statusCode == 201) {
          stopwatch.stop();
          Logger.performance(
              '[MARKETPLACE] updateMemberWithVendorDetails completed successfully in ${stopwatch.elapsedMilliseconds}ms');
          return const Right(true);
        } else {
          Logger.error(
              '[MARKETPLACE] Server error with status code ${response.statusCode}');
  ...
  Future<bool> isMemberVendor(String memberId) async {
    try {
      ...
      if (user == null) {
        Logger.error(
            '[MARKETPLACE] User not found in database when checking vendor status');
        return false;
      }
  ...
  Future<Either<Failure, Vendor>> createVendor({
    ...
    // Log the input parameters
    Logger.data(
        '[MARKETPLACE] Create vendor parameters: memberId=$memberId, businessName=$businessName, email=$email, phone=$phone');
    Logger.data(
        '[MARKETPLACE] Description: ${description.length > 50 ? '${description.substring(0, 50)}...' : description}');
    Logger.data(
  ...
  Future<Either<Failure, Product>> getProduct(String id) async {
    ...
    return _executeAuthenticatedRequest<Product>(
      request: (token) async {
        ...
        Logger.data(
            '[MARKETPLACE] Sending GET request to $_baseUrl/getProduct/$id');
        final response = await _loggedRequest(
          () => _httpClient.get(
        ...
        if (response.statusCode == 200) {
          ...
          stopwatch.stop();
          Logger.performance(
              '[MARKETPLACE] getProduct completed successfully in ${stopwatch.elapsedMilliseconds}ms');
  ...
  Future<Either<Failure, List<Product>>> searchProducts(
    ...
    final stopwatch = Stopwatch()..start();
    Logger.data(
        '[MARKETPLACE] Starting searchProducts operation for query: $query');
        ...
        if (response.statusCode == 200) {
          ...
          try {
            if (responseData.containsKey('data') &&
              ...
              stopwatch.stop();
              Logger.performance(
                  '[MARKETPLACE] searchProducts completed successfully in ${stopwatch.elapsedMilliseconds}ms');
          ...
          Logger.error(
              '[MARKETPLACE] Server error with status code ${response.statusCode}');
          return Left(
              ServerFailure('Failed to search products: ${response.body}'));
        }
      },
    );
  }
  ...
  Future<Either<Failure, String>> createInternalAccount({
    ...
    return _executeAuthenticatedRequest<String>(
      request: (token) async {
        ...
        Logger.data(
            '[MARKETPLACE] Sending POST request to $_baseUrl/createAccountInternal');
        final response = await _loggedRequest(
          () => _httpClient.post(
        ...
        if (response.statusCode == 200 || response.statusCode == 201) {
          ...
          stopwatch.stop();
          Logger.performance(
              '[MARKETPLACE] createInternalAccount completed successfully in ${stopwatch.elapsedMilliseconds}ms');
  ...
  Future<Either<Failure, Product>> createProduct({
    ...
    if (productAccountId.isEmpty) {
      Logger.data(
          '[MARKETPLACE] No account ID provided, creating internal account');
      ...
      Logger.data(
          '[MARKETPLACE] Created internal account with ID: $productAccountId');
    }

    final finalProductAccountId =
        productAccountId; // Create a final copy for the closure
    ...
    return _executeAuthenticatedRequest<Product>(
      request: (token) async {
        ...
        Logger.data(
            '[MARKETPLACE] Sending POST request to $_baseUrl/createProduct');
        final response = await _loggedRequest(
          () => _httpClient.post(
        ...
        if (response.statusCode == 200 || response.statusCode == 201) {
          ...
          try {
            if (responseData.containsKey('data') &&
              ...
              final productData =
                  responseData['data']['product'] as Map<String, dynamic>;
              ...
              stopwatch.stop();
              Logger.performance(
                  '[MARKETPLACE] createProduct completed successfully in ${stopwatch.elapsedMilliseconds}ms');
              return Right(product);
            } else {
              ...
              stopwatch.stop();
              Logger.performance(
                  '[MARKETPLACE] createProduct completed with temporary product in ${stopwatch.elapsedMilliseconds}ms');
              return Right(product);
            }
          } catch (e) {
            Logger.error(
                '[MARKETPLACE] Error parsing product data from response', e);
          ...
          Logger.error(
              '[MARKETPLACE] Server error with status code ${response.statusCode}');
          return Left(
              ServerFailure('Failed to create product: ${response.body}'));
        }
      },
    );
  }
  ...
  Future<Either<Failure, Product>> updateProduct({
    ...
    return _executeAuthenticatedRequest<Product>(
      request: (token) async {
        ...
        Logger.data(
            '[MARKETPLACE] Sending PUT request to $_baseUrl/updateProduct');
        final response = await _loggedRequest(
          () => _httpClient.put(
        ...
        if (response.statusCode == 200 || response.statusCode == 204) {
          ...
          if (response.statusCode == 204 || response.body.isEmpty) {
            Logger.data(
                '[MARKETPLACE] No content in response, fetching updated product');
          ...
          try {
            if (responseData.containsKey('data') &&
              ...
              stopwatch.stop();
              Logger.performance(
                  '[MARKETPLACE] updateProduct completed successfully in ${stopwatch.elapsedMilliseconds}ms');
              return Right(product);
            } else {
          ...
          Logger.error(
              '[MARKETPLACE] Server error with status code ${response.statusCode}');
          return Left(
              ServerFailure('Failed to update product: ${response.body}'));
        }
      },
    );
  }
  ...
  Future<Either<Failure, Invoice>> getInvoice(String id) async {
    ...
    return _executeAuthenticatedRequest<Invoice>(
      request: (token) async {
        ...
        Logger.data(
            '[MARKETPLACE] Sending GET request to $_baseUrl/getInvoice/$id');
        final response = await _loggedRequest(
          () => _httpClient.get(
        ...
        if (response.statusCode == 200) {
          ...
          try {
            ...
            if (responseData.containsKey('data') &&
              ...
              invoiceDetails = responseData['data']['action']['details']
                  as Map<String, dynamic>;
              Logger.data(
                  '[MARKETPLACE] Found invoice details in data.action.details');
            }
            ...
            if (invoiceDetails == null &&
              ...
              invoiceDetails = responseData['data']['dashboard']['invoice']
                  as Map<String, dynamic>;
              Logger.data(
                  '[MARKETPLACE] Found invoice details in data.dashboard.invoice');
            }
            ...
            if (responseData.containsKey('data') &&
              ...
              try {
                createdAt =
                    DateTime.parse(responseData['data']['action']['timestamp']);
              } catch (e) {
            ...
            if (invoiceDetails.containsKey('lines') &&
                invoiceDetails['lines'] is List) {
            ...
            stopwatch.stop();
            Logger.performance(
                '[MARKETPLACE] getInvoice completed successfully in ${stopwatch.elapsedMilliseconds}ms');
            return Right(invoice);
          } catch (e) {
            Logger.error(
                '[MARKETPLACE] Error parsing invoice data from response', e);
  ...
  Future<Either<Failure, List<Invoice>>> getInvoicesByBuyer(
    ...
    Logger.data(
        '[MARKETPLACE] getInvoicesByBuyer called with buyer ID: $buyerId');
    return const Right([]);
  }
  ...
  Future<Either<Failure, List<Invoice>>> getInvoicesByVendor(
    ...
    // Empty implementation - API not yet available
    Logger.data(
        '[MARKETPLACE] getInvoicesByVendor called with vendor ID: $vendorId');
    return const Right([]);
  }
  ...
  Future<Either<Failure, Invoice>> createInvoice({
    ...
    Logger.data(
        '[INVOICE_API] [$correlationId] Starting createInvoice operation');
    ...
    Logger.data(
        '[INVOICE_API] [$correlationId] - lineItems count: ${lineItems.length}');
    Logger.data(
        '[INVOICE_API] [$correlationId] - totalAmount: $totalAmount (${totalAmount / 100} in dollars)');
    Logger.data('[INVOICE_API] [$correlationId] - currency: $currency');
    Logger.data(
        '[INVOICE_API] [$correlationId] - paymentMethod: $paymentMethod');
    Logger.data(
        '[INVOICE_API] [$correlationId] - notes: ${notes != null ? '${notes.length} characters' : 'null'}');

    // Log line items details
    for (var i = 0; i < lineItems.length; i++) {
      final item = lineItems[i];
      Logger.data(
          '[INVOICE_API] [$correlationId] Line item #${i + 1}: productId=${item.productId}, name=${item.productName}, quantity=${item.quantity}, unitPrice=${item.unitPrice}, totalPrice=${item.totalPrice}');
    }
    ...
    final user = await _databaseHelper.getUser();
    final userRetrievalDuration =
        DateTime.now().difference(userRetrievalStart).inMilliseconds;
    Logger.data(
        '[INVOICE_API] [$correlationId] User retrieval took $userRetrievalDuration ms');
    ...
    if (user.dashboard != null && user.dashboard!.accounts.isNotEmpty) {
      Logger.data(
          '[INVOICE_API] [$correlationId] User has ${user.dashboard!.accounts.length} accounts');
      ...
      for (final account in user.dashboard!.accounts) {
        if (account.accountType == 'PERSONAL') {
          ...
          Logger.data(
              '[INVOICE_API] [$correlationId] Found PERSONAL account: $paymentAccountId (${account.accountName})');
          break;
        }
      }
      ...
      if (paymentAccountId.isEmpty) {
        for (final account in user.dashboard!.accounts) {
          if (account.accountName.toUpperCase().contains('PERSONAL')) {
            paymentAccountId = account.accountID;
            Logger.data(
                '[INVOICE_API] [$correlationId] Found account with PERSONAL in name: $paymentAccountId (${account.accountName})');
            break;
          }
        }
      }
    ...
    Logger.data(
        '[INVOICE_API] [$correlationId] Using payment account ID: $paymentAccountId');
    ...
    return _executeAuthenticatedRequest<Invoice>(
      request: (token) async {
        ...
        Logger.data(
            '[INVOICE_API] [$correlationId] Converting line items to API format');
        final items = lineItems
            .map((item) => {
                  'accountID': item.productId,
                  'amount':
                      item.totalPrice / 100, // Convert from cents to dollars
                })
            .toList();

        // Log currency conversion
        Logger.data(
            '[INVOICE_API] [$correlationId] Currency conversion: $totalAmount cents -> ${totalAmount / 100} dollars');
        ...
        final requestBody = jsonEncode(requestData);
        Logger.data(
            '[INVOICE_API] [$correlationId] Request body prepared: ${requestBody.length} characters');

        // Call the /generateInvoice API endpoint with authentication
        Logger.data(
            '[INVOICE_API] [$correlationId] Sending POST request to $_baseUrl/generateInvoice');
        ...
        final apiCallDuration =
            DateTime.now().difference(apiCallStart).inMilliseconds;

        // Log response details
        Logger.data(
            '[INVOICE_API] [$correlationId] API call completed in $apiCallDuration ms');
        Logger.data(
            '[INVOICE_API] [$correlationId] Response content length: ${response.contentLength ?? 'unknown'} bytes');

        if (response.statusCode == 200 || response.statusCode == 201) {
          Logger.data(
              '[INVOICE_API] [$correlationId] Response successful (${response.statusCode})');

          // Parse the response body
          final responseBodyStart = DateTime.now();
          final responseData = jsonDecode(response.body);
          final responseBodyDuration =
              DateTime.now().difference(responseBodyStart).inMilliseconds;
          Logger.data(
              '[INVOICE_API] [$correlationId] Response body parsing took $responseBodyDuration ms');

          // Log response structure
          Logger.data(
              '[INVOICE_API] [$correlationId] Response structure: ${_getResponseStructure(responseData)}');

          // Extract invoice details from the response according to the specified format
          String invoiceId = '';
          String invoiceQrLink = '';
          DateTime createdAt = DateTime.now();
          int extractedTotalAmount =
              totalAmount; // Default to the input total amount
          String extractedDenomination =
              currency; // Default to the input currency

          try {
            Logger.data(
                '[INVOICE_API] [$correlationId] Starting response data extraction');
            ...
            if (responseData.containsKey('data') &&
                ...
                responseData['data']['action'] is Map<String, dynamic>) {
              Logger.data(
                  '[INVOICE_API] [$correlationId] Found data.action structure in response');
              final action =
                  responseData['data']['action'] as Map<String, dynamic>;

              // Extract timestamp
              if (action.containsKey('timestamp') &&
                  action['timestamp'] is String) {
                createdAt = DateTime.parse(action['timestamp']);
                Logger.data(
                    '[INVOICE_API] [$correlationId] Extracted timestamp: ${action['timestamp']}');
              } else {
                Logger.data(
                    '[INVOICE_API] [$correlationId] No timestamp found in response, using current time');
              }

              // Extract details
              if (action.containsKey('details') &&
                  action['details'] is Map<String, dynamic>) {
                Logger.data(
                    '[INVOICE_API] [$correlationId] Found details object in response');
                final details = action['details'] as Map<String, dynamic>;

                // Log all available fields in details for debugging
                Logger.data(
                    '[INVOICE_API] [$correlationId] Available fields in details: ${details.keys.join(', ')}');

                // Extract invoice ID
                if (details.containsKey('invoiceID')) {
                  invoiceId = details['invoiceID']?.toString() ?? '';
                  Logger.data(
                      '[INVOICE_API] [$correlationId] Extracted invoice ID: $invoiceId');
                } else {
                  Logger.error(
                      '[INVOICE_API] [$correlationId] No invoiceID field found in details');
                }
                ...
                if (details.containsKey('invoiceQRLink')) {
                  ...
                  Logger.data(
                      '[INVOICE_API] [$correlationId] Extracted invoice QR link: $invoiceQrLink');
                } else {
                  Logger.data(
                      '[INVOICE_API] [$correlationId] No invoiceQRLink field found in details');
                }

                // Extract total amount
                if (details.containsKey('totalAmount') &&
                    details['totalAmount'] is num) {
                  ...
                  Logger.data(
                      '[INVOICE_API] [$correlationId] Extracted total amount: $rawAmount dollars -> $extractedTotalAmount cents');
                } else {
                  Logger.data(
                      '[INVOICE_API] [$correlationId] No totalAmount field found in details, using input amount: $totalAmount cents');
                }

                // Extract denomination (currency)
                if (details.containsKey('denomination')) {
                  extractedDenomination =
                      details['denomination']?.toString() ?? currency;
                  Logger.data(
                      '[INVOICE_API] [$correlationId] Extracted denomination: $extractedDenomination');
                } else {
                  Logger.data(
                      '[INVOICE_API] [$correlationId] No denomination field found in details, using input currency: $currency');
                }
              } else {
                Logger.error(
                    '[INVOICE_API] [$correlationId] No details object found in action');
              }
            } else {
              Logger.error(
                  '[INVOICE_API] [$correlationId] Response does not contain expected data.action structure');
              Logger.error(
                  '[INVOICE_API] [$correlationId] Response top-level keys: ${responseData.keys.join(', ')}');
            }
            ...
            Logger.data(
                '[INVOICE_API] [$correlationId] Data extraction completed successfully');
          } catch (e, stackTrace) {
            Logger.error(
                '[INVOICE_API] [$correlationId] Error extracting invoice details from response',
                e,
                stackTrace);
            Logger.error(
                '[INVOICE_API] [$correlationId] Error type: ${e.runtimeType}');
            Logger.error(
                '[INVOICE_API] [$correlationId] Response body: ${response.body}');
            return Left(ServerFailure('Failed to parse invoice response: $e'));
          }

          // Create an Invoice object from the response data
          Logger.data(
              '[INVOICE_API] [$correlationId] Creating Invoice object with extracted data');
          final invoice = Invoice(
            id: invoiceId,
            vendorId: vendorId,
            lineItems: lineItems,
          ...
          stopwatch.stop();
          Logger.performance(
              '[INVOICE_API] [$correlationId] createInvoice completed successfully in ${stopwatch.elapsedMilliseconds}ms');

          // Return the invoice with the QR link as an additional property
          return Right(invoice);
        } else if (response.statusCode == 400) {
          Logger.error(
              '[INVOICE_API] [$correlationId] Invalid input data with status code 400');
          Logger.error(
              '[INVOICE_API] [$correlationId] Response body: ${response.body}');
          ...
          try {
            ...
            if (errorData.containsKey('error') &&
              ...
              Logger.error(
                  '[INVOICE_API] [$correlationId] Error message: ${errorData['error']}');
            }
          } catch (e) {
            Logger.error(
                '[INVOICE_API] [$correlationId] Could not parse error response',
                e);
          }

          return Left(
              ValidationFailure('Invalid input data: ${response.body}'));
        } else {
          Logger.error(
              '[INVOICE_API] [$correlationId] Server error with status code ${response.statusCode}');
          Logger.error(
              '[INVOICE_API] [$correlationId] Response body: ${response.body}');
          return Left(
              ServerFailure('Failed to create invoice: ${response.body}'));
        }
      },
    ).catchError((e, stackTrace) {
      stopwatch.stop();
      Logger.error(
          '[INVOICE_API] [$correlationId] Unhandled exception during invoice creation',
          e,
          stackTrace);
      Logger.error(
          '[INVOICE_API] [$correlationId] Exception type: ${e.runtimeType}');
  ...
  Future<Either<Failure, List<AssetMarker>>> getAssetMarkersByProduct(
    ...
    // Empty implementation - API not yet available
    Logger.data(
        '[MARKETPLACE] getAssetMarkersByProduct called with product ID: $productId');
    return const Right([]);
  }
  ...
  Future<Either<Failure, List<AssetMarker>>> getAssetMarkersByOwner(
    ...
    // Empty implementation - API not yet available
    Logger.data(
        '[MARKETPLACE] getAssetMarkersByOwner called with owner ID: $ownerId');
    return const Right([]);
  }
  ...
  Future<Either<Failure, Map<String, String>>> uploadProfileImage({
    ...
    Logger.data(
        '[PROFILE_IMAGE] Image file size: ${fileSizeKB.toStringAsFixed(2)} KB (${fileSizeMB.toStringAsFixed(2)} MB)');
    ...
    Logger.data(
        '[PROFILE_IMAGE] Image converted to base64: ${base64SizeKB.toStringAsFixed(2)} KB (${base64SizeMB.toStringAsFixed(2)} MB)');

    // Check if base64 encoded data is too large
    if (base64SizeKB > 13653) {
    ...
    Logger.data(
        '[PROFILE_IMAGE] Request prepared with payload size: ${requestSizeKB.toStringAsFixed(2)} KB (${requestSizeMB.toStringAsFixed(2)} MB)');
    ...
    return _executeAuthenticatedRequest<Map<String, String>>(
      request: (token) async {
        ...
        Logger.data(
            '[PROFILE_IMAGE] Sending POST request to $_baseUrl/uploadAndOptimizeJpg');
        final response = await _loggedRequest(
          () => _httpClient.post(
        ...
        if (response.statusCode == 200 || response.statusCode == 201) {
          ...
          // Then try to extract from data.action.details object (actual API response format)
            ...
            Logger.data(
                '[PROFILE_IMAGE] Asset IDs extracted from data.action.details object');
          }
          ...
          Logger.data(
              '[PROFILE_IMAGE] Asset IDs extracted: original=$originalAssetId, 200px=$asset200Id, 600px=$asset600Id');

          // Log warning if any asset ID is empty
          if (originalAssetId.isEmpty ||
              asset200Id.isEmpty ||
              asset600Id.isEmpty) {
            Logger.error('[PROFILE_IMAGE] One or more asset IDs are empty',
          ...
          stopwatch.stop();
          Logger.performance(
              '[PROFILE_IMAGE] uploadProfileImage completed successfully in ${stopwatch.elapsedMilliseconds}ms');
          ...
          Logger.error(
              '[PROFILE_IMAGE] Server error with status code ${response.statusCode}');
      ...
      stopwatch.stop();
      Logger.error(
          '[PROFILE_IMAGE] Error uploading profile image', e, stackTrace);
  ...
  Future<Either<Failure, bool>> updateProfilePictures({
    ...
    return _executeAuthenticatedRequest<bool>(
      request: (token) async {
        ...
        Logger.data(
            '[PROFILE_IMAGE] Sending POST request to $_baseUrl/updateProfilePics');
        final response = await _loggedRequest(
          () => _httpClient.post(
        ...
        if (response.statusCode == 200 || response.statusCode == 201) {
          stopwatch.stop();
          Logger.performance(
              '[PROFILE_IMAGE] updateProfilePictures completed successfully in ${stopwatch.elapsedMilliseconds}ms');
          return const Right(true);
        } else {
          Logger.error(
              '[PROFILE_IMAGE] Server error with status code ${response.statusCode}');
          return Left(ServerFailure(
              'Failed to update profile pictures: ${response.body}'));
        }
      },
    ).catchError((e, stackTrace) {
      stopwatch.stop();
      Logger.error(
          '[PROFILE_IMAGE] Error updating profile pictures', e, stackTrace);
  ...
  Future<Either<Failure, bool>> updateStoreStatus({
    ...
    Logger.data(
        '[STORE_STATUS] Parameters: accountId=$accountId, storeOpen=$storeOpen, location=(${latitude ?? 'null'}, ${longitude ?? 'null'})');
    ...
    return _executeAuthenticatedRequest<bool>(
      request: (token) async {
        ...
        Logger.data(
            '[STORE_STATUS] Sending POST request to $_baseUrl/storeStatus/$accountId');
        final response = await _loggedRequest(
          () => _httpClient.post(
        ...
        if (response.statusCode == 200 || response.statusCode == 201) {
          ...
          if (responseData.containsKey('message')) {
            Logger.data(
                '[STORE_STATUS] Response message: ${responseData['message']}');
          }
          ...
          try {
            ...
            if (user != null) {
              ...
              await _databaseHelper.saveUser(updatedUser);
              Logger.data(
                  '[STORE_STATUS] Updated user with storeOpen: $storeOpen and location: (${latitude ?? 'null'}, ${longitude ?? 'null'})');
            }
          } catch (e) {
            Logger.error(
                '[STORE_STATUS] Error updating user with store status', e);
            // Continue even if update fails, as the API call was successful
          }

          stopwatch.stop();
          Logger.performance(
              '[STORE_STATUS] updateStoreStatus completed successfully in ${stopwatch.elapsedMilliseconds}ms');
          return const Right(true);
        } else {
          Logger.error(
              '[STORE_STATUS] Server error with status code ${response.statusCode}');
          return Left(
              ServerFailure('Failed to update store status: ${response.body}'));
        }
      },
    );
  }
  ...
  @override
  Future<Either<Failure, Map<String, dynamic>>> getStorefront(
      String accountId) async {
    final stopwatch = Stopwatch()..start();
    Logger.data(
        '[STOREFRONT] Starting getStorefront operation for account ID: $accountId');
    ...
    return _executeAuthenticatedRequest<Map<String, dynamic>>(
      request: (token) async {
        ...
        if (response.statusCode == 200) {
          ...
          if (responseData.containsKey('message')) {
            Logger.data(
                '[STOREFRONT] Response message: ${responseData['message']}');
          }
          ...
          if (responseData.containsKey('data')) {
            ...
            stopwatch.stop();
            Logger.performance(
                '[STOREFRONT] getStorefront completed successfully in ${stopwatch.elapsedMilliseconds}ms');
            return Right(data);
          } else {
