...
class CreditRatingDisplay extends StatelessWidget {
  ...
  Widget _buildBreakdownRow(String label, String description, double amount, Color color) {
  ...
  Map<String, int> _calculatePercentages() {
    ...
    int redeemedPercent = total > 0
        ? ((creditRating!.redeemedTotalUSD / total) * 100).round()
        : 0;
    int outstandingPercent = total > 0
        ? ((creditRating!.outstandingTotalUSD / total) * 100).round()
        : 0;
    int defaultedPercent = total > 0
        ? ((creditRating!.defaultedTotalUSD / total) * 100).round()
        : 0;
    int writtenOffPercent = total > 0
        ? ((creditRating!.writtenOffTotalUSD / total) * 100).round()
        : 0;

    // Ensure minimum visibility for non-zero values
    if (creditRating!.redeemedTotalUSD > 0 && redeemedPercent == 0)
      redeemedPercent = 1;
    if (creditRating!.outstandingTotalUSD > 0 && outstandingPercent == 0)
      outstandingPercent = 1;
    if (creditRating!.defaultedTotalUSD > 0 && defaultedPercent == 0)
      defaultedPercent = 1;
    if (creditRating!.writtenOffTotalUSD > 0 && writtenOffPercent == 0)
      writtenOffPercent = 1;

    // Adjust percentages to ensure they sum to 100%
    final sum = redeemedPercent +
        outstandingPercent +
        defaultedPercent +
        writtenOffPercent;
    if (sum != 100) {
